// <auto-generated />
using System;
using LevelUpGym.Api.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace LevelUpGym.Api.Migrations
{
    [DbContext(typeof(LevelUpDbContext))]
    [Migration("20260826200620_AddGoalsTableAndRelationships")]
    partial class AddGoalsTableAndRelationships
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.8")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("LevelUpGym.Api.Models.Auth", b =>
                {
                    b.Property<int>("IdAuth")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdAuth"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<int>("IdProfile")
                        .HasColumnType("int");

                    b.Property<byte[]>("Password")
                        .IsRequired()
                        .HasColumnType("varbinary(max)");

                    b.Property<byte[]>("PasswordSalt")
                        .IsRequired()
                        .HasColumnType("varbinary(max)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdAuth");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.HasIndex("IdProfile")
                        .IsUnique();

                    b.ToTable("auth", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.CashMovement", b =>
                {
                    b.Property<int>("IdMovimiento")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdMovimiento"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Descripcion")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<DateTime?>("Fecha")
                        .HasColumnType("datetime2");

                    b.Property<decimal>("Monto")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Tipo")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdMovimiento");

                    b.ToTable("CashMovements");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.ClassEnrollment", b =>
                {
                    b.Property<int>("IdEnrollment")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdEnrollment"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<int>("IdClass")
                        .HasColumnType("int");

                    b.Property<int>("IdCliente")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdEnrollment");

                    b.HasIndex("IdCliente");

                    b.HasIndex("IdClass", "IdCliente")
                        .IsUnique();

                    b.ToTable("class_enrollments", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.ClassSession", b =>
                {
                    b.Property<int>("IdClass")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdClass"));

                    b.Property<int>("CapacidadMaxima")
                        .HasColumnType("int");

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateOnly>("Fecha")
                        .HasColumnType("date");

                    b.Property<TimeSpan>("HoraInicio")
                        .HasColumnType("time");

                    b.Property<int>("IdEntrenador")
                        .HasColumnType("int");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdClass");

                    b.HasIndex("IdEntrenador");

                    b.ToTable("class_sessions", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Client", b =>
                {
                    b.Property<int>("IdCliente")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdCliente"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<int>("IdProfile")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdCliente");

                    b.HasIndex("IdProfile")
                        .IsUnique();

                    b.ToTable("clientes", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.EmpleadoRolGimnasio", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<int>("IdEmpleado")
                        .HasColumnType("int");

                    b.Property<int>("IdRolGym")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("IdRolGym");

                    b.HasIndex("IdEmpleado", "IdRolGym")
                        .IsUnique();

                    b.ToTable("roles_empleados", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Employee", b =>
                {
                    b.Property<int>("IdEmpleado")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdEmpleado"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Descripcion")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("Especialidad")
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateOnly?>("FechaContratacion")
                        .HasColumnType("date");

                    b.Property<int?>("IdEps")
                        .HasColumnType("int");

                    b.Property<int>("IdProfile")
                        .HasColumnType("int");

                    b.Property<decimal?>("SalarioBase")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdEmpleado");

                    b.HasIndex("IdEps");

                    b.HasIndex("IdProfile")
                        .IsUnique();

                    b.ToTable("empleados", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.EmployeePayment", b =>
                {
                    b.Property<int>("IdPago")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdPago"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Descripcion")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int>("EmployeeIdEmpleado")
                        .HasColumnType("int");

                    b.Property<DateOnly>("FechaPago")
                        .HasColumnType("date");

                    b.Property<int>("IdEmpleado")
                        .HasColumnType("int");

                    b.Property<decimal>("Monto")
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdPago");

                    b.HasIndex("EmployeeIdEmpleado");

                    b.ToTable("EmployeePayments");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Eps", b =>
                {
                    b.Property<int>("IdEps")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdEps"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdEps");

                    b.ToTable("EpsList");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Goal", b =>
                {
                    b.Property<int>("IdObjetivo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdObjetivo"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Descripcion")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateOnly>("FechaInicio")
                        .HasColumnType("date");

                    b.Property<DateOnly?>("FechaLimite")
                        .HasColumnType("date");

                    b.Property<int>("IdCliente")
                        .HasColumnType("int");

                    b.Property<int>("IdTipoObjetivo")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.Property<decimal>("ValorMeta")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)");

                    b.HasKey("IdObjetivo");

                    b.HasIndex("IdCliente");

                    b.HasIndex("IdTipoObjetivo");

                    b.ToTable("Goals", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.GoalType", b =>
                {
                    b.Property<int>("IdTipoObjetivo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdTipoObjetivo"));

                    b.Property<bool>("Activo")
                        .HasColumnType("bit");

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Descripcion")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("Direccion")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TipoDato")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("Unidad")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdTipoObjetivo");

                    b.HasIndex("Nombre")
                        .IsUnique();

                    b.ToTable("GoalTypes", (string)null);

                    b.HasData(
                        new
                        {
                            IdTipoObjetivo = 1,
                            Activo = true,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Descripcion = "Alcanzar un peso corporal determinado.",
                            Direccion = "MENOR",
                            Nombre = "Peso",
                            TipoDato = "DECIMAL",
                            Unidad = "kg"
                        },
                        new
                        {
                            IdTipoObjetivo = 2,
                            Activo = true,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Descripcion = "Alcanzar un índice de masa corporal determinado.",
                            Direccion = "MENOR",
                            Nombre = "IMC",
                            TipoDato = "DECIMAL",
                            Unidad = "-"
                        },
                        new
                        {
                            IdTipoObjetivo = 3,
                            Activo = true,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Descripcion = "Reducir la medida de cintura.",
                            Direccion = "MENOR",
                            Nombre = "Cintura",
                            TipoDato = "DECIMAL",
                            Unidad = "cm"
                        },
                        new
                        {
                            IdTipoObjetivo = 4,
                            Activo = true,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Descripcion = "Aumentar la medida de pecho.",
                            Direccion = "MAYOR",
                            Nombre = "Pecho",
                            TipoDato = "DECIMAL",
                            Unidad = "cm"
                        },
                        new
                        {
                            IdTipoObjetivo = 5,
                            Activo = true,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Descripcion = "Aumentar la medida de brazo.",
                            Direccion = "MAYOR",
                            Nombre = "Brazo",
                            TipoDato = "DECIMAL",
                            Unidad = "cm"
                        },
                        new
                        {
                            IdTipoObjetivo = 6,
                            Activo = true,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Descripcion = "Aumentar la medida de pierna.",
                            Direccion = "MAYOR",
                            Nombre = "Pierna",
                            TipoDato = "DECIMAL",
                            Unidad = "cm"
                        },
                        new
                        {
                            IdTipoObjetivo = 7,
                            Activo = true,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Descripcion = "Reducir el porcentaje de grasa corporal.",
                            Direccion = "MENOR",
                            Nombre = "Porcentaje de grasa",
                            TipoDato = "DECIMAL",
                            Unidad = "%"
                        });
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Item", b =>
                {
                    b.Property<int>("IdItem")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdItem"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("Tipo")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdItem");

                    b.ToTable("items", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Membership", b =>
                {
                    b.Property<int>("IdMembresia")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdMembresia"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Descripcion")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<int?>("IdItem")
                        .HasColumnType("int");

                    b.Property<string>("Nombre")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<decimal?>("Precio")
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdMembresia");

                    b.HasIndex("IdItem")
                        .IsUnique()
                        .HasFilter("[IdItem] IS NOT NULL");

                    b.HasIndex("Nombre")
                        .IsUnique()
                        .HasFilter("[Nombre] IS NOT NULL");

                    b.ToTable("membresias", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.MensajeContacto", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Asunto")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Correo")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)")
                        .HasDefaultValue("Pendiente");

                    b.Property<DateTime>("FechaEnvio")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValueSql("GETUTCDATE()");

                    b.Property<string>("Mensaje")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("nvarchar(2000)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.ToTable("mensajes_contacto", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Permission", b =>
                {
                    b.Property<int>("IdPermiso")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdPermiso"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Nombre")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdPermiso");

                    b.ToTable("Permissions");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Profile", b =>
                {
                    b.Property<int>("IdProfile")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdProfile"));

                    b.Property<string>("Apellidos")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<decimal?>("Estatura")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("NumDocumento")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<decimal?>("Peso")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Sexo")
                        .HasMaxLength(9)
                        .HasColumnType("nvarchar(9)");

                    b.Property<string>("Telefono")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("TipoDocumento")
                        .IsRequired()
                        .HasMaxLength(2)
                        .HasColumnType("nvarchar(2)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdProfile");

                    b.HasIndex("NumDocumento")
                        .IsUnique();

                    b.ToTable("profiles", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Progress", b =>
                {
                    b.Property<int>("IdProgreso")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdProgreso"));

                    b.Property<string>("Altura")
                        .HasMaxLength(4)
                        .HasColumnType("nvarchar(4)");

                    b.Property<decimal?>("Brazo")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal?>("Cintura")
                        .HasColumnType("decimal(18,2)");

                    b.Property<int>("ClientIdCliente")
                        .HasColumnType("int");

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateOnly?>("FechaMedicion")
                        .HasColumnType("date");

                    b.Property<int>("IdCliente")
                        .HasColumnType("int");

                    b.Property<string>("Imc")
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<decimal?>("Pecho")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal?>("Peso")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal?>("Pierna")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal?>("PorcentajeGrasa")
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdProgreso");

                    b.HasIndex("ClientIdCliente");

                    b.ToTable("ProgressReports");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.RolGimnasio", b =>
                {
                    b.Property<int>("IdRolGym")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdRolGym"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Descripcion")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdRolGym");

                    b.HasIndex("Nombre")
                        .IsUnique();

                    b.ToTable("roles_gimnasio", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Role", b =>
                {
                    b.Property<int>("IdRol")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdRol"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Descripcion")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("Nombre")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdRol");

                    b.ToTable("Roles");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.RolePermission", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<int?>("IdPermiso")
                        .HasColumnType("int");

                    b.Property<int?>("IdRol")
                        .HasColumnType("int");

                    b.Property<int?>("PermissionIdPermiso")
                        .HasColumnType("int");

                    b.Property<int?>("RoleIdRol")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("PermissionIdPermiso");

                    b.HasIndex("RoleIdRol");

                    b.HasIndex("IdRol", "IdPermiso")
                        .IsUnique()
                        .HasFilter("[IdRol] IS NOT NULL AND [IdPermiso] IS NOT NULL");

                    b.ToTable("roles_permisos", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Subscription", b =>
                {
                    b.Property<int>("IdSuscripcion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdSuscripcion"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateOnly>("FechaFin")
                        .HasColumnType("date");

                    b.Property<DateOnly>("FechaInicio")
                        .HasColumnType("date");

                    b.Property<int>("IdCliente")
                        .HasColumnType("int");

                    b.Property<int>("IdEstado")
                        .HasColumnType("int");

                    b.Property<int>("IdMembresia")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdSuscripcion");

                    b.HasIndex("IdCliente");

                    b.HasIndex("IdEstado");

                    b.HasIndex("IdMembresia");

                    b.ToTable("suscripciones", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.SubscriptionStatus", b =>
                {
                    b.Property<int>("IdEstado")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdEstado"));

                    b.Property<string>("Concepto")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("IdEstado");

                    b.ToTable("SubscriptionStatuses");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.UserRole", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("AuthIdAuth")
                        .HasColumnType("int");

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<int?>("IdAuth")
                        .HasColumnType("int");

                    b.Property<int?>("IdRol")
                        .HasColumnType("int");

                    b.Property<int?>("RoleIdRol")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("AuthIdAuth");

                    b.HasIndex("RoleIdRol");

                    b.HasIndex("IdAuth", "IdRol")
                        .IsUnique()
                        .HasFilter("[IdAuth] IS NOT NULL AND [IdRol] IS NOT NULL");

                    b.ToTable("usuarios_roles", (string)null);
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Auth", b =>
                {
                    b.HasOne("LevelUpGym.Api.Models.Profile", "Profile")
                        .WithOne("Auth")
                        .HasForeignKey("LevelUpGym.Api.Models.Auth", "IdProfile")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Profile");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.ClassEnrollment", b =>
                {
                    b.HasOne("LevelUpGym.Api.Models.ClassSession", "ClassSession")
                        .WithMany("Enrollments")
                        .HasForeignKey("IdClass")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LevelUpGym.Api.Models.Client", "Client")
                        .WithMany()
                        .HasForeignKey("IdCliente")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ClassSession");

                    b.Navigation("Client");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.ClassSession", b =>
                {
                    b.HasOne("LevelUpGym.Api.Models.Employee", "Entrenador")
                        .WithMany()
                        .HasForeignKey("IdEntrenador")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Entrenador");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Client", b =>
                {
                    b.HasOne("LevelUpGym.Api.Models.Profile", "Profile")
                        .WithOne("Client")
                        .HasForeignKey("LevelUpGym.Api.Models.Client", "IdProfile")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Profile");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.EmpleadoRolGimnasio", b =>
                {
                    b.HasOne("LevelUpGym.Api.Models.Employee", "Employee")
                        .WithMany("EmpleadoRoles")
                        .HasForeignKey("IdEmpleado")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LevelUpGym.Api.Models.RolGimnasio", "RolGimnasio")
                        .WithMany("EmpleadoRoles")
                        .HasForeignKey("IdRolGym")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Employee");

                    b.Navigation("RolGimnasio");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Employee", b =>
                {
                    b.HasOne("LevelUpGym.Api.Models.Eps", "Eps")
                        .WithMany("Employees")
                        .HasForeignKey("IdEps");

                    b.HasOne("LevelUpGym.Api.Models.Profile", "Profile")
                        .WithOne("Employee")
                        .HasForeignKey("LevelUpGym.Api.Models.Employee", "IdProfile")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Eps");

                    b.Navigation("Profile");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.EmployeePayment", b =>
                {
                    b.HasOne("LevelUpGym.Api.Models.Employee", "Employee")
                        .WithMany("Payments")
                        .HasForeignKey("EmployeeIdEmpleado")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Employee");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Goal", b =>
                {
                    b.HasOne("LevelUpGym.Api.Models.Client", "Client")
                        .WithMany("Goals")
                        .HasForeignKey("IdCliente")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LevelUpGym.Api.Models.GoalType", "GoalType")
                        .WithMany("Goals")
                        .HasForeignKey("IdTipoObjetivo")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Client");

                    b.Navigation("GoalType");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Membership", b =>
                {
                    b.HasOne("LevelUpGym.Api.Models.Item", "Item")
                        .WithOne("Membership")
                        .HasForeignKey("LevelUpGym.Api.Models.Membership", "IdItem");

                    b.Navigation("Item");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Progress", b =>
                {
                    b.HasOne("LevelUpGym.Api.Models.Client", "Client")
                        .WithMany("ProgressReports")
                        .HasForeignKey("ClientIdCliente")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Client");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.RolePermission", b =>
                {
                    b.HasOne("LevelUpGym.Api.Models.Permission", "Permission")
                        .WithMany("RolePermissions")
                        .HasForeignKey("PermissionIdPermiso");

                    b.HasOne("LevelUpGym.Api.Models.Role", "Role")
                        .WithMany("RolePermissions")
                        .HasForeignKey("RoleIdRol");

                    b.Navigation("Permission");

                    b.Navigation("Role");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Subscription", b =>
                {
                    b.HasOne("LevelUpGym.Api.Models.Client", "Client")
                        .WithMany("Subscriptions")
                        .HasForeignKey("IdCliente")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LevelUpGym.Api.Models.SubscriptionStatus", "Status")
                        .WithMany("Subscriptions")
                        .HasForeignKey("IdEstado")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LevelUpGym.Api.Models.Membership", "Membership")
                        .WithMany("Subscriptions")
                        .HasForeignKey("IdMembresia")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Client");

                    b.Navigation("Membership");

                    b.Navigation("Status");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.UserRole", b =>
                {
                    b.HasOne("LevelUpGym.Api.Models.Auth", "Auth")
                        .WithMany("UserRoles")
                        .HasForeignKey("AuthIdAuth");

                    b.HasOne("LevelUpGym.Api.Models.Role", "Role")
                        .WithMany("UserRoles")
                        .HasForeignKey("RoleIdRol");

                    b.Navigation("Auth");

                    b.Navigation("Role");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Auth", b =>
                {
                    b.Navigation("UserRoles");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.ClassSession", b =>
                {
                    b.Navigation("Enrollments");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Client", b =>
                {
                    b.Navigation("Goals");

                    b.Navigation("ProgressReports");

                    b.Navigation("Subscriptions");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Employee", b =>
                {
                    b.Navigation("EmpleadoRoles");

                    b.Navigation("Payments");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Eps", b =>
                {
                    b.Navigation("Employees");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.GoalType", b =>
                {
                    b.Navigation("Goals");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Item", b =>
                {
                    b.Navigation("Membership");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Membership", b =>
                {
                    b.Navigation("Subscriptions");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Permission", b =>
                {
                    b.Navigation("RolePermissions");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Profile", b =>
                {
                    b.Navigation("Auth");

                    b.Navigation("Client");

                    b.Navigation("Employee");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.RolGimnasio", b =>
                {
                    b.Navigation("EmpleadoRoles");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.Role", b =>
                {
                    b.Navigation("RolePermissions");

                    b.Navigation("UserRoles");
                });

            modelBuilder.Entity("LevelUpGym.Api.Models.SubscriptionStatus", b =>
                {
                    b.Navigation("Subscriptions");
                });
#pragma warning restore 612, 618
        }
    }
}
